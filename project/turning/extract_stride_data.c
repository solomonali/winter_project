...
int main(int argc, char **argv)
{
	 ...
	 * of a stride.
	 */
	S_i = (float *) malloc(sizeof(float) * n_T); // P
	float *S_min;
	S_min = (float *) malloc(sizeof(float) * n_P); //T    //for min (troughs)

        int a , b , c;
	a = 0; b = 1; n_S = 0; c = 0; 
	while( b < n_T)                     //P
	{
		if((T_i[b] - T_i[a]) > 300)   //P
		{
			S_i[n_S] = T_i[a];         //P
		        S_min[n_S] = P_i[a];	  // T
			n_S++;
			...
			if((b == n_T) && ((T_i[b-1] - T_i[a-1])>300)) //P
		        {
				S_i[n_S] = T_i[b-1];	//P
				S_min[n_S] = P_i[b-1];  //T
				n_S++;
